...
import re
from utils import find_first_boolean, string_to_boolean, find_first_digit
...
def binary_choice_match(prediction, ground_truth):
    prediction = prediction.lower()
    bool_index = find_first_boolean(prediction)
    if bool_index is not None:
        return string_to_boolean(bool_index) == ground_truth
    else:
        return False

def multiple_choice_match(prediction, ground_truth):
    prediction = prediction.lower()
    int_index = find_first_digit(prediction)
    if int_index is not None:
        if type(int_index) != type(ground_truth):
            int_index = int(int_index)
        return int_index == ground_truth
    else:
        return "No choice id is outputted"